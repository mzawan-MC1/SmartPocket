-- Ensure the managed-people loan repayment RPC enforces the existing
-- managed_people subscription entitlement even though it runs as SECURITY DEFINER.
...
BEGIN
  ...
  IF NOT public.subscription_feature_enabled_for_user(v_user_id, 'managed_people') THEN
    RAISE EXCEPTION 'Managed People is not available in your current subscription';
  END IF;
