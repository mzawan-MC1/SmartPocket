...
import { CalendarClock, CreditCard, Edit2, Eye, Filter, Loader2, MoreVertical, Pause, Play, Trash2, Wallet, XCircle } from 'lucide-react';
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-y-2.5 sm:gap-y-3">
              {filteredSubscriptions.map((subscription) => {
                ...
                const nextChargeValue = formatDateValue(subscription.next_billing_date) || notAvailableLabel;
                const accountValue = subscription.account?.name || t('personalSubscriptions.labels.unlinked', { ns: 'portal' });
                const freqLabel = t('personalSubscriptions.form.fields.billingFrequency', { ns: 'portal' });
                const acctLabel = t('personalSubscriptions.labels.paymentAccount', { ns: 'portal' });
                const nextLabel = t('personalSubscriptions.labels.nextCharge', { ns: 'portal' });
                const providerSubtitle = `${subscription.provider || t('personalSubscriptions.labels.customProvider', { ns: 'portal' })} · ${subscription.category?.name || t('transactions.noCategory', { ns: 'portal' })}`;
                return (
                  <article
                    key={subscription.id}
                    className={`group/item relative rounded-[15px] border border-border/85 bg-card shadow-[0_1px_2px_rgba(15,23,42,0.05),0_1px_1px_rgba(15,23,42,0.03)] transition-all duration-150 hover:border-accent/30 hover:shadow-[0_4px_14px_rgba(15,23,42,0.06)] py-2.5 px-2.5 sm:py-2.5 sm:px-3 ${isRTL ? 'sm:text-right' : 'sm:text-left'}`}
                  >
                        ...
                        <div className={`flex items-center justify-end sm:justify-end gap-1.5 w-full sm:w-auto ${isRTL ? 'sm:flex-row-reverse flex-row-reverse' : ''}`}>
                          ...
                          <div className="relative">
                            ...
                            {openMenuId === subscription.id ? (
                              <div
                                role="menu"
                                className={`absolute top-full z-[60] mt-1.5 flex w-[min(13rem,calc(100vw-2.25rem))] flex-col overflow-hidden rounded-[18px] border border-border bg-card p-1.5 shadow-card-lg sm:min-w-[12.5rem] sm:w-auto ${isRTL ? 'left-0' : 'right-0'}`}
                              >
